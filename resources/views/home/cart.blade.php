...
{{--                                                <span class="text-muted">{{ $attr->name }}: </span> {{ $attr->pivot->value_id }}--}}
                                                <span class="text-muted">{{ $attr->name }}: </span> {{ \App\Models\AttributeValue::find($attr->pivot->value_id)->value }}
                                                &nbsp;&nbsp;
                                            @endforeach
                                        </small>
                                    </div>
                            ...
                            <td class="align-middle p-4">
                                <select name="" class="form-control text-center">
                                    @foreach(range(1 , $cart['product']->inventory) as $opt)
                                        <option value="{{ $opt }}" {{ $cart['quantity'] == $opt ? 'selected' : '' }}>{{ $opt }}</option>
                                    @endforeach
                                </select>
                            </td>
                ...
                <div class="d-flex flex-wrap justify-content-between align-items-center pb-4">
                    ...
                    <div class="d-flex">
                        ...
                        <div class="text-right mt-4">
                            ...
                            <div class="text-large"><strong> تومان{{  \App\Helpers\Cart\Cart::all()->sum(function ($cart) {return $cart['product']->price * $cart['quantity'];}) }}</strong></div>
                        </div>
                    </div>
                </div>
